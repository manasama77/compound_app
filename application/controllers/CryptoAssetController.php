...
class CryptoAssetController extends CI_Controller
{
	...
	public function index()
	{
		...
		$data = [
			'title'              => APP_NAME . ' | Crypto Asset - Paket Kamu',
	...
	public function add()
	{
		...
		$where_member_crypto_asset = ['id_member' => $this->id_member];
		...
		$data = [
			'title'        => APP_NAME . ' | Crypto Asset - Join Paket',
	...
	public function pick($id_konfigurasi_crypto_asset = null)
	{
		...
		$id_konfigurasi_crypto_asset = str_replace(UYAH, '', base64_decode($id_konfigurasi_crypto_asset));

		$where                    = ['id' => $id_konfigurasi_crypto_asset];
		$id_package_crypto_asset = $this->M_core->get('konfigurasi_crypto_asset', 'id_package_crypto_asset', $where)->row()->id_package_crypto_asset;

		$where_check = [
			'id_member' 	=> $this->id_member,
			'id_package >' 	=> $id_package_crypto_asset,
			'state !=' 		=> 'cancel',
			'deleted_at' 	=> null,
		];
		...
		$arr = $this->M_crypto_asset->get_package($id_konfigurasi_crypto_asset);

		$data = [
			'title'                        => APP_NAME . ' | Pilih Paket Crypto Asset',
			'content'                      => 'crypto_asset/pick',
			'vitamin_js'                   => 'crypto_asset/pick_js',
			'id_package_crypto_asset'     => base64_encode(UYAH . $id_package_crypto_asset),
			'id_konfigurasi_crypto_asset' => base64_encode(UYAH . $id_konfigurasi_crypto_asset),
			'arr'                          => $arr,
			'csrf'                         => $this->csrf,
		];
		$this->template->render($data);
	...
	public function checkout_coinpayment()
	{
		...
		$arr_package = $this->M_crypto_asset->get_package($id_config);
	...
	protected function _create_transaction($data)
	{
		...
		$req['ipn_url']     = site_url('coinpayment/ipn/crypto_asset/' . $data['invoice']);
